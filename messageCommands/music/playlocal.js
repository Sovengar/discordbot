...
module.exports = {
    ...
    run: async (client, message, args) => {
        if(message.author.id !== client.config.owner_id) return message.reply("Only my creator can use this command!")

        const channel = message.member.voice.channel;
        if(!channel) return message.reply("You are not in a voice channel");

        const fileName = args.join(" ") + '.mp3';
        if(!fileName) return message.reply("Provide a file name!");
